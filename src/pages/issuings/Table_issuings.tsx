...
import { getIssuings, deleteIssuing } from "../../utils/issuings";
...
const TableIssuing = () => {
  ...
  const fetchData = async () => {
    ...
    setErrorMsg(null);
    try {
      const payload = await getIssuings();
      if (!Array.isArray(payload)) throw new Error("Response bukan array. Cek API backend!");
      setData(payload);
    } catch (err: any) {
      console.error(err);
      setErrorMsg(err?.message || "Network error saat mengambil data issuing");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number | string) => {
    if (!window.confirm("Yakin ingin menghapus issuing ini?")) return;
    try {
      await deleteIssuing(id);
      fetchData();
    } catch (err) {
      console.error(err);
      alert("Gagal menghapus issuing!");
    }
  };
      ...
      cell: ({ row }) => (
        <HStack gap={2}>
          <Button
            size="sm"
            colorScheme="blue"
            onClick={() => navigate(`/updateissuings/${row.original.id}`)}
          >
            Edit
          </Button>
          <Button
  ...
  if (loading)
    return (
      <Box textAlign="center" py={20}>
        <Spinner size="xl" />
        <Text mt={4}>Sedang memuat data issuing...</Text>
      </Box>
    );
  ...
  if (errorMsg)
    return (
      <Box p={4}>
        <Text color="red.500" mb={2}>Gagal mengambil data issuing!</Text>
    ...
    <Box overflowX="auto">
      <HStack mb={4}>
        ...
        <Button
          ...
          color="white"
          size="sm"
          onClick={() => navigate("/createissuings")}
        >
          Tambah Issuing
        </Button>
      </HStack>
