// src/pages/issuing/UpdateIssuing.tsx
...
import { getIssuingById, updateIssuing } from "../../utils/issuings";
...
const UpdateIssuing = () => {
  ...
  useEffect(() => {
    const fetchIssuing = async () => {
      try {
        const data = await getIssuingById(id!);
        setForm({
          number: data.number || "",
        ...
        console.error(err);
        alert("Gagal mengambil data issuing!");
      }
    };
  ...
  const handleSubmit = async () => {
    try {
      ...
      await updateIssuing(id!, payload);
      alert("Issuing berhasil diperbarui!");
      navigate("/issuings");
    } catch (err) {
      console.error(err);
      alert("Gagal memperbarui issuing!");
    }
  };
